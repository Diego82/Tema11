...
public class LecturaFichero {
	...
	public static void main(String[] args)throws FileNotFoundException {
		...
		//Si existe argumento en la linea de comandos entrara en este if
		if (args.length != 0) {
			...
			//Primero recorremos la lista para averiguar la longitud del nombre mas largo y mas corto
			int tamanoMenor=lista.get(0).length();
			int tamanoMayor = lista.get(0).length();
			for (int i = 0; i < lista.size(); i++) {
				if (tamanoMenor>lista.get(i).length()) {
					tamanoMenor = lista.get(i).length();
				}
				if (tamanoMayor<lista.get(i).length()) {
					tamanoMayor = lista.get(i).length();
				}
			}
